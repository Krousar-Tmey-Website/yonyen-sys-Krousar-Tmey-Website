...
@section('description', 'Latest press articles, media coverage, and news featuring Krousar Thmey\'s work in Cambodia.')
...
<div class="bg-white py-16 md:py-24">
    <div class="max-w-7xl mx-auto px-6 text-center">
        ...
        {{-- Title --}}
        @php
            $mediaTitle = \App\Models\HomeSetting::getValue('media_title', 'MEDIA');
        ...
        <h1 class="text-5xl md:text-7xl font-bold text-[#1a4fa0] mb-8 tracking-tight">{{ $mediaTitle }}</h1>

        {{-- Social share icons --}}
        <div class="flex items-center justify-center gap-3 mb-6">
            {{-- Facebook --}}
            <a href="{{ $mediaFacebookUrl }}" target="_blank" rel="noopener noreferrer"
               class="w-11 h-11 rounded-full bg-[#1a4fa0]/10 hover:bg-[#1a4fa0] text-[#1a4fa0] hover:text-white flex items-center justify-center transition-all duration-200 hover:scale-110">
            ...
            <a href="{{ $mediaLinkedinUrl }}" target="_blank" rel="noopener noreferrer"
               class="w-11 h-11 rounded-lg bg-[#1a4fa0]/10 hover:bg-[#1a4fa0] text-[#1a4fa0] hover:text-white flex items-center justify-center transition-all duration-200 hover:scale-110">
            ...
            <button onclick="if(navigator.share){navigator.share({title:'Krousar Thmey Media',url:window.location.href})}else{navigator.clipboard.writeText(window.location.href);this.classList.add('bg-green-500','text-white');setTimeout(()=>this.classList.remove('bg-green-500','text-white'),1500)}"
               class="w-11 h-11 rounded-lg bg-[#1a4fa0]/10 hover:bg-[#1a4fa0] text-[#1a4fa0] hover:text-white flex items-center justify-center transition-all duration-200 hover:scale-110 cursor-pointer">
...
{{-- ──────────────────────────────────────────────────── --}}
{{-- "KROUSAR THMEY IN THE NEWS" — FEATURED MEDIA ITEM  --}}
{{-- ──────────────────────────────────────────────────── --}}
@if($featured)
<section class="pb-20 bg-white">
    <div class="max-w-7xl mx-auto px-6">
        <h2 class="text-[#1a4fa0] font-bold text-sm uppercase tracking-[0.15em] mb-8">Krousar Thmey In The News</h2>
            ...
            <div class="grid md:grid-cols-2 gap-0">
                ...
                <div class="p-8 lg:p-10 xl:p-12 flex flex-col justify-center">
                    ...
                    <div class="text-[#1a4fa0] text-xs font-bold uppercase tracking-[0.12em] mb-5">
                        {{ strtoupper($featured->source) }}
                    </div>
                    @endif

                    <h3 class="text-2xl lg:text-3xl font-bold text-[#1a4fa0] leading-tight mb-4">
                        &ldquo;{{ $featured->title }}&rdquo;
                    </h3>

                    <p class="text-gray-500 italic text-sm mb-6">
                        published {{ $featured->published_at?->format('m.d.y') ?? $featured->created_at->format('m.d.y') }}
                    </p>
                    ...
                    <a href="{{ $featured->external_link }}" target="_blank" rel="noopener noreferrer"
...
<section class="pb-20 bg-white border-t border-gray-100">
    <div class="max-w-7xl mx-auto px-6">
        <h2 class="text-[#1a4fa0] font-bold text-sm uppercase tracking-[0.15em] mb-2">Media Coverage</h2>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <article class="border border-gray-200 bg-white flex flex-col group hover:shadow-lg transition-shadow duration-300">
                ...
                <div class="p-6 flex flex-col flex-1">
                    ...
                    <h3 class="font-bold text-[#1a4fa0] text-base leading-snug mb-3 line-clamp-2">
                        {{ $item->title }}
                    </h3>
...
{{-- FALLBACK                                             --}}
{{-- ──────────────────────────────────────────────────── --}}
@if(!$featured && $mediaItems->isEmpty())
<section class="py-20 bg-white border-t border-gray-100">
    <div class="max-w-7xl mx-auto px-6 text-center py-16">
        ...
        <p class="text-gray-400 text-sm max-w-md mx-auto">Check back soon for press articles and media coverage of Krousar Thmey.</p>
