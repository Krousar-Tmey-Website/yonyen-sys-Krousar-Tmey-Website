            ...
            <div class="flex items-center gap-5">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @foreach($socialLinks as $social)
                        <a href="{{ $social->url }}" target="_blank" rel="noopener" aria-label="{{ $social->platform_name }}"
                           class="text-white/50 hover:text-[#8da83a] transition-colors">
                            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">{!! $topBarSvgs[$social->icon] ?? '<path d="M21 13v10h-6v-6h-6v6h-6v-10h-3l12-12 12 12h-3z"/>' !!}</svg>
                        </a>
                    @endforeach
                </div>
                <a href="{{ route('donate') }}"
                    ...
                    <div class="flex items-center gap-3 mt-6">
                        ...
                        <a href="{{ $social->url }}" target="_blank" rel="noopener" aria-label="{{ $social->platform_name }}"
                           class="w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center hover:bg-[#8da83a] transition-colors">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">{!! $footerSvgs[$social->icon] ?? '<path d="M21 13v10h-6v-6h-6v6h-6v-10h-3l12-12 12 12h-3z"/>' !!}</svg>
                        </a>
                        @endforeach
                    </div>
                </div>
