    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 hover-lift">
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-bold text-gray-800 text-sm">Donation Trends <span class="text-gray-400 font-normal">{{ $chartData['year'] }}</span></h3>
                    <p class="text-xs text-gray-400">{{ $chartData['chartSubtitle'] }}</p>
                </div>
            </div>
            <select onchange="window.location.href = '?year=' + this.value"
                class="text-xs border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-[#e8a020] bg-white text-gray-600 font-medium">
                @foreach($chartData['availableYears'] as $y)
                <option value="{{ $y }}" {{ $chartData['year'] == $y ? 'selected' : '' }}>{{ $y }}</option>
                @endforeach
            </select>
        </div>
