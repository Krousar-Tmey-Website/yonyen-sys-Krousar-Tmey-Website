...
@section('breadcrumb', 'Manage the statistics displayed on the homepage')
                    ...
                    <div class="px-5 py-4 bg-gray-50 border-t border-gray-100" x-show="editing" x-cloak>
                        <form action="{{ route('admin.impact-statistics.update', $stat) }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                            ...
                            <div>
                                ...
                                <input type="file" name="image_file" accept="image/*"
                                       class="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#2d6fa3] mb-2">
                                <p class="text-gray-400 text-xs mb-1">Current: {{ $stat->image ? Str::limit($stat->image, 30) : 'Default image' }}</p>
                                <input type="url" name="image" value="{{ $stat->image }}"
                                       class="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#2d6fa3]"
                                       placeholder="https://example.com/image.jpg">
                            </div>
                            <div class="grid grid-cols-2 gap-3">
