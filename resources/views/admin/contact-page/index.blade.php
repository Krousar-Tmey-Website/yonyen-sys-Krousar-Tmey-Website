...
@section('breadcrumb', 'Support → Contact')
...
<div class="space-y-8" x-data="{ tab: 'banner' }">
    ...
    {{-- ═══════════════════════════════════════════ --}}
    <div x-show="tab === 'inquiries'" x-cloak class="space-y-6 max-w-6xl mx-auto">

        {{-- Search & Filter --}}
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <form method="GET" action="{{ route('admin.contact-page.index') }}" id="searchForm">
                <div class="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        @if (request('name') || request('status'))
                            <a href="{{ route('admin.contact-page.index', ['tab' => 'inquiries']) }}"
                            ...
                            @foreach ($inquiries as $inquiry)
                                <tr class="hover:bg-gray-50/50 transition-colors {{ $inquiry->Status === 'New' ? 'bg-blue-50/30' : '' }}">
                                    ...
                                    <td class="px-6 py-4 text-gray-400 text-xs whitespace-nowrap">
                                        {{ $inquiry->ReceivedDate->format('d M Y') }}
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <a href="{{ route('admin.contacts.show', $inquiry) }}"
                                                 class="action-btn btn-view" title="View Inquiry">
                                                 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                 </svg>
                                             </a>
                                             <form method="POST" action="{{ route('admin.contacts.destroy', $inquiry) }}" class="inline">
                                                 @csrf @method('DELETE')
                                                 <button type="submit"
                                                     onclick="return confirm('Are you sure you want to delete this message?');"
                                                     class="action-btn btn-delete" title="Delete Inquiry">
                                                     <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
