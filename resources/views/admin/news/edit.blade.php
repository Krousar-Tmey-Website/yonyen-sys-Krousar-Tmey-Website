    ...
    <form id="articleEditForm" action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data" class="space-y-6" data-news-ajax-form>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
            ...
            <div class="form-group" x-show="lang === 'fr'" x-cloak>
                ...
                <input type="text" name="title_fr" value="{{ old('title_fr', $news->title_fr) }}"
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
            ...
            <div class="form-group form-group--no-margin" x-show="lang === 'en'">
                @include('admin.news._content-editor', ['name' => 'content', 'contentValue' => $news->content])
                @error('content')<div class="form-error">{{ $message }}</div>@enderror
            </div>
            <div class="form-group form-group--no-margin" x-show="lang === 'fr'" x-cloak>
                @include('admin.news._content-editor', ['name' => 'content_fr', 'contentValue' => $news->content_fr, 'placeholder' => 'Rédigez le contenu en français ici...'])
                ...
                <div class="links-container" id="tagLinksContainer">
                    @if(!empty($news->tag_links))
                        ...
                        <div class="link-item">
                            ...
                            <button type="button" class="remove-link" data-index="{{ $loop->index }}" onclick="removeTagLink(this.dataset.index)" title="Remove tag">×</button>
                        </div>
                        @endforeach
                    @else
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
                ...
                <div class="video-preview-list">
                    ...
                    <div class="video-preview-item" id="videoItem{{ $index }}">
                        ...
                        <button type="button" class="remove-gallery-item"
                                ...
                                onclick="toggleRemoveVideoItem(this.dataset.index, this.dataset.path)" title="Remove">×</button>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="form-group">
                ...
                <div class="upload-area" onclick="document.getElementById('videosInput').click()">
                    ...
                    <div class="upload-subtitle">Max 500MB each. MP4, MOV, or WebM.</div>
                </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <button type="submit" form="delete-news-form" class="text-red-400 hover:text-red-600 text-sm ml-auto">Delete Article</button>
        </div>
    </form>

    {{-- Delete Form (separate from main form) --}}
    <form id="delete-news-form" action="{{ route('admin.news.destroy', $news) }}" method="POST" class="hidden"
...
let tagLinks = @json(!empty($news->tag_links) ? $news->tag_links : []);
