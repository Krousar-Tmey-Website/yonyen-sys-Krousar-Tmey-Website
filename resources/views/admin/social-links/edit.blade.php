...
@section('breadcrumb', 'Update social media link details')

@section('content')

<div class="max-w-2xl">
    <form action="{{ route('admin.social-links.update', $socialLink) }}" method="POST" class="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
        @csrf @method('PUT')

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                <label class="block text-sm font-medium text-gray-600 mb-1.5">Platform Name *</label>
                <input type="text" name="platform_name" value="{{ old('platform_name', $socialLink->platform_name) }}"
            ...
            <div>
                ...
                <input type="text" name="icon" value="{{ old('icon', $socialLink->icon) }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $socialLink->sort_order) }}" min="0"
                       class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2d6fa3]/20 focus:border-[#2d6fa3]">
            </div>
        ...
        <div class="flex items-center gap-3 pt-2">
            <button type="submit" class="btn-primary">Update Social Link</button>
