...
@section('title', 'Edit Page Item')
@section('page-title', 'Edit Page Item')
@section('breadcrumb')
    <a href="{{ route('admin.program-page-items.index') }}" class="hover:text-[#2d6fa3] transition-colors">Page Items</a> / Edit
@endsection

@section('content')
<div class="max-w-4xl">
<form action="{{ route('admin.program-page-items.update', $programPageItem) }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="p-6 space-y-6">
        ...
        <div>
            ...
            <select name="program_page_id"
                ...
                @foreach($pages as $p)
                <option value="{{ $p->id }}" {{ old('program_page_id', $programPageItem->program_page_id) == $p->id ? 'selected' : '' }}>
                    {{ $p->title }}
                </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $programPageItem->title) }}" required
                   class="w-full rounded-xl border-gray-300 focus:border-[#2d6fa3] focus:ring focus:ring-[#2d6fa3]/20 transition-all text-sm">
        ...
        <div>
            ...
            <textarea name="short_content" rows="3"
                      class="w-full rounded-xl border-gray-300 focus:border-[#2d6fa3] focus:ring focus:ring-[#2d6fa3]/20 transition-all text-sm"
                      placeholder="Brief description that appears on the card listing...">{{ old('short_content', $programPageItem->short_content) }}</textarea>
        ...
        <div>
            ...
            <textarea name="detail_content" rows="14"
                      ...
                      placeholder="Full content (HTML is supported)...">{{ old('detail_content', $programPageItem->detail_content) }}</textarea>
        ...
        <div>
            ...
            @foreach([
                ['label' => 'Primary Image', 'field' => 'image', 'url_field' => 'image_url', 'current' => $programPageItem->image_url, 'hasImage' => (bool)$programPageItem->image],
                ['label' => 'Image 2', 'field' => 'image_2', 'url_field' => 'image_2_url', 'current' => $programPageItem->image_2_url, 'hasImage' => (bool)$programPageItem->image_2],
                ['label' => 'Image 3', 'field' => 'image_3', 'url_field' => 'image_3_url', 'current' => $programPageItem->image_3_url, 'hasImage' => (bool)$programPageItem->image_3],
            ] as $img)
            <div class="mb-5 pb-5 border-b border-gray-50 last:border-0">
                <div class="font-medium text-xs text-gray-600 mb-2">{{ $img['label'] }}</div>
                @if($img['hasImage'])
                <img src="{{ $img['current'] }}" class="h-20 rounded-lg object-cover mb-2 border border-gray-200">
                @endif
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Upload new (replaces current)</label>
        ...
        <div class="grid grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $programPageItem->sort_order) }}" min="0"
                       class="w-full rounded-xl border-gray-300 focus:border-[#2d6fa3] focus:ring focus:ring-[#2d6fa3]/20 transition-all text-sm">
            </div>
            <div class="flex items-end pb-1">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $programPageItem->is_active) ? 'checked' : '' }}
