...
<form action="{{ route('admin.program-pages.update', $item) }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="p-6 space-y-6">
        ...
        <div>
            ...
            <textarea name="short_content" data-ckeditor rows="3"
                      class="w-full rounded-xl border-gray-300 focus:border-[#2d6fa3] focus:ring focus:ring-[#2d6fa3]/20 transition-all text-sm">{{ old('short_content', $item->short_content) }}</textarea>
        ...
        <div>
            ...
            <textarea name="detail_content" data-ckeditor rows="14"
                      class="w-full rounded-xl border-gray-300 focus:border-[#2d6fa3] focus:ring focus:ring-[#2d6fa3]/20 transition-all text-sm font-mono">{{ old('detail_content', $item->detail_content) }}</textarea>
        ...
        <div>
            ...
            @foreach([
                ['label' => 'Primary Image', 'field' => 'image',   'url_field' => 'image_url',   'current' => $item->image_url,   'has' => (bool)$item->image],
                ['label' => 'Image 2',       'field' => 'image_2', 'url_field' => 'image_2_url', 'current' => $item->image_2_url, 'has' => (bool)$item->image_2],
                ['label' => 'Image 3',       'field' => 'image_3', 'url_field' => 'image_3_url', 'current' => $item->image_3_url, 'has' => (bool)$item->image_3],
            ] as $img)
            <div class="mb-5 pb-5 border-b border-gray-50 last:border-0">
                <div class="font-medium text-xs text-gray-600 mb-2">{{ $img['label'] }}</div>
                @if($img['has'])
                <img src="{{ $img['current'] }}" class="h-20 w-auto rounded-lg object-cover mb-2 border border-gray-200" alt="Current">
                <p class="text-xs text-gray-400 mb-2">Upload or enter a URL to replace. Leave blank to keep current.</p>
                @endif
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Upload new</label>
                        <input type="file" name="{{ $img['field'] }}" accept="image/*"
                               class="w-full text-sm text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-[#2d6fa3]/10 file:text-[#2d6fa3] hover:file:bg-[#2d6fa3]/20">
                    </div>
                    <div>
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div class="flex items-end pb-1">
                <label class="flex items-center gap-2 cursor-pointer">
                    ...
                    <span class="text-sm font-medium text-gray-700">Publish (visible on site)</span>
                </label>
            </div>
        </div>
    </div>
    <div class="bg-gray-50 px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.program-pages.index') }}" class="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-900">Cancel</a>
