...
<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
        @if($item->image)
        <img src="{{ str_starts_with($item->image, 'http') ? $item->image : asset('storage/' . $item->image) }}" alt="{{ $item->title }}" class="w-full h-36 object-cover">
        @else
        ...
        <div class="p-5">
            ...
            @if($item->title)
            <p class="text-gray-400 text-xs leading-relaxed mb-4 line-clamp-2">{{ $item->title }}</p>
            @endif
            <div class="flex items-center justify-between mt-4">
                ...
                <form action="{{ route('admin.gallery.destroy', $item) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this Gallery?');" class="inline-block">
                    @csrf
                    @method('DELETE')
