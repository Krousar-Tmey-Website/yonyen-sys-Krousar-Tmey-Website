...
<div class="flex h-full">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div x-data="{ show: true }" x-show="show" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
            ...
            <button @click="show = false" type="button" class="flex-shrink-0 p-0.5 rounded-md hover:bg-green-200/50 transition-colors">
        ...
        <div x-data="{ show: true }" x-show="show" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
            ...
            <button @click="show = false" type="button" class="flex-shrink-0 p-0.5 rounded-md hover:bg-red-200/50 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        @endif
