            ...
            <form action="{{ route('admin.presentation.banner.update') }}" method="POST" enctype="multipart/form-data" class="space-y-5" x-data="bilingualForm()">
                ...
                <div class="bg-gray-50/60 rounded-xl p-5 border border-gray-100/80">
                    ...
                    <div class="mb-4 flex items-center gap-4 p-3 bg-white rounded-xl border border-gray-200 shadow-sm">
                        <img src="{{ $pbImageUrl }}" alt="Current banner" class="h-16 w-24 object-cover rounded-lg border border-gray-200">
                        <div class="flex-1 min-w-0">
                            <p class="text-xs font-medium text-gray-700 truncate">{{ basename($pbImage) }}</p>
                    ...
                    <div class="border-2 border-dashed border-gray-200 rounded-xl p-5 text-center hover:border-[#2d6fa3]/50 transition-all duration-200 cursor-pointer bg-white hover:bg-blue-50/30 group"
                         ...
                         @drop.prevent="$el.classList.remove('border-[#2d6fa3]', 'bg-blue-50/40'); const f = $event.dataTransfer.files[0]; if(f) { $refs.fileInput.files = $event.dataTransfer.files; fileName = f.name; const r = new FileReader(); r.onload = e => { document.getElementById('banner-preview').style.backgroundImage = 'url(' + e.target.result + ')'; document.getElementById('banner-preview').querySelector('.absolute').style.backgroundImage = 'url(' + e.target.result + ')'; }; r.readAsDataURL(f); }"
                         @click="$refs.fileInput.click()">
                        <input type="file" name="presentation_banner_image" accept="image/png,image/jpg,image/jpeg,image/webp,image/svg+xml"
                               class="hidden" x-ref="fileInput"
                               @change="fileName = $event.target.files[0]?.name || ''; if($event.target.files.length){let r=new FileReader();r.onload=function(e){document.getElementById('banner-preview').style.backgroundImage='url('+e.target.result+')';document.getElementById('banner-preview').querySelector('.absolute').style.backgroundImage='url('+e.target.result+')'};r.readAsDataURL($event.target.files[0])}">
                    ...
                    <div class="mt-3" x-data="{ showUrl: {{ $pbImage && !str_starts_with($pbImage, 'http') ? 'false' : 'true' }} }">
                ...
                <div class="bg-gray-50/60 rounded-xl p-5 border border-gray-100/80">
                    <div class="flex items-center gap-2.5 mb-4">
                        ...
                        <span class="text-sm font-semibold text-gray-700">Overlay Color &amp; Badge</span>
                    </div>
                    <div class="grid sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1.5">Overlay Color</label>
                            <div class="flex items-center gap-3">
                                <input type="color" id="presentation_overlay_color_picker" name="presentation_banner_overlay_color" value="{{ $pbOverlay }}"
                                       onchange="document.getElementById('presentation_overlay_color_text').value=this.value;document.getElementById('banner-preview').style.backgroundColor=this.value"
                                       class="h-11 w-14 shrink-0 rounded-lg border border-gray-200 bg-white p-1 cursor-pointer">
                                <input type="text" id="presentation_overlay_color_text" name="presentation_banner_overlay_color_text" value="{{ $pbOverlay }}"
                                       oninput="document.getElementById('presentation_overlay_color_picker').value=this.value;document.getElementById('banner-preview').style.backgroundColor=this.value"
                                       class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-xs font-mono focus:outline-none focus:ring-2 focus:ring-[#2d6fa3]/20 focus:border-[#2d6fa3]"
                                       placeholder="#1a3c6e">
                            </div>
                        </div>
                        <div>
                ...
                <div>
                    ...
                    <div x-show="lang === 'fr'" x-cloak>
                        <x-admin.rich-text name="presentation_banner_title_fr" :value="old('presentation_banner_title_fr', $settings['presentation_banner_title_fr'] ?? '')" lang="fr" :rows="1"
                                           placeholder="Krousar Thmey, la première organisation cambodgienne…" />
...
<div x-show="lang === 'en'">
                                ...
                                <input type="text" name="label" x-model="statLabel"
                                       class="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2d6fa3]/20 focus:border-[#2d6fa3] transition-shadow"
