            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                ...
                <div class="flex-[2] w-full">
                    ...
                    <input type="text" name="name" id="name" value="{{ request('name') }}"
                        placeholder="Search sender name..." autocomplete="off"
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        @if ($inquiries->isEmpty())
            <div class="py-16 text-center text-gray-400">
                ...
                <p class="text-sm font-medium text-gray-500">No contact inquiries found</p>
                <p class="text-xs mt-1">Messages sent through the Contact page will appear here.</p>
                @if (request('name') || request('status'))
                    <a href="{{ route('admin.contacts.index') }}"
                        class="text-[#2d6fa3] text-sm hover:underline mt-2 inline-block">Clear filters</a>
                @endif
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($inquiries as $inquiry)
                            <tr
                                class="hover:bg-gray-50/50 transition-colors {{ $inquiry->Status === 'New' ? 'bg-blue-50/30' : '' }}">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-8 h-8 rounded-full bg-[#2d6fa3]/10 flex items-center justify-center flex-shrink-0">
                                            <span
                                                class="text-xs font-bold text-[#2d6fa3]">{{ substr($inquiry->Name, 0, 2) }}</span>
                                        </div>
                                        <span
                                            class="{{ $inquiry->Status === 'New' ? 'font-semibold text-gray-800' : 'font-medium text-gray-700' }}">{{ $inquiry->Name }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-gray-400 text-xs whitespace-nowrap">
                                    {{ $inquiry->ReceivedDate->format('d M Y') }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ route('admin.contacts.show', $inquiry) }}"
                                            class="inline-flex items-center justify-center text-xs font-medium text-blue-500 hover:text-blue-600 bg-blue-50 hover:bg-blue-100 w-8 h-8 rounded-full transition-all"
                                            title="View Inquiry">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <form method="POST" action="{{ route('admin.contacts.destroy', $inquiry) }}">
                                            ...
                                            <button type="submit"
                                                class="inline-flex items-center justify-center text-xs font-medium text-red-500 hover:text-red-600 bg-red-50 hover:bg-red-100 w-8 h-8 rounded-full transition-all"
