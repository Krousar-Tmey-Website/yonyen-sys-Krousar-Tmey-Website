...
@section('breadcrumb', 'Create a new sponsor')

@section('content')

<div class="mb-8 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-gray-900">Add New Sponsor</h2>
        <p class="mt-1 text-sm text-gray-500">Create a new sponsor profile to display on the homepage.</p>
    </div>
        ...
        <form action="{{ route('admin.sponsors.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-8 tracking-tight">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-[#1a3c6e] focus:ring-[#1a3c6e] px-4 py-2.5 transition-colors" placeholder="e.g. Ministry of Education">
                ...
                <div class="bg-gray-50/50 p-6 rounded-xl border border-gray-100">
                    <label class="block text-sm font-semibold text-gray-800 mb-4">Sponsor Logo <span class="text-gray-400 font-normal ml-1">(Choose one method)</span></label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
                        ...
                        <div class="flex flex-col justify-center">
                            <label for="logo_file" class="block text-sm font-medium text-gray-700 mb-2">Upload Image File</label>
                        ...
                        <div class="flex flex-col justify-center">
                            ...
                            <input type="url" name="logo_url" id="logo_url" value="{{ old('logo_url') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-[#1a3c6e] focus:ring-[#1a3c6e] px-4 py-2.5 transition-colors" placeholder="https://example.com/logo.png">
                            @error('logo_url')<p class="text-red-500 text-xs mt-2 font-medium">{{ $message }}</p>@enderror
                        </div>
                    </div>
                    <p class="text-gray-500 text-xs mt-6 flex items-center"><svg class="w-4 h-4 mr-1.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> For best results, use a transparent PNG under 2MB.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', 0) }}" class="w-full md:w-32 rounded-lg border-gray-300 shadow-sm focus:border-[#1a3c6e] focus:ring-[#1a3c6e] px-4 py-2.5 transition-colors">
