...
@section('page-title', 'Create Sponsor')
@section('breadcrumb', 'Sponsors → Create')
    ...
    <form action="{{ route('admin.sponsors.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 lg:p-8 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="form-group form-group--no-margin">
                      ...
                      <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" class="form-control w-full md:w-32">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 lg:p-8 space-y-4">
            <div class="flex items-center justify-between">
                ...
                <span class="text-xs text-gray-400">Choose one method</span>
            </div>

            <div class="bg-gray-50/50 p-6 rounded-xl border border-gray-200/50">
                <div class="flex items-center justify-end mb-4">
                    <!-- Toggle Tab buttons -->
                    <div class="flex items-center gap-1 p-1 bg-gray-200/60 border border-gray-200 rounded-lg">
                        <button type="button"
                ...
                <div x-show="logoMethod === 'file'"
                    ...
                    <div class="border-2 border-dashed border-gray-200 hover:border-indigo-400 rounded-xl p-6 bg-white hover:bg-gray-50/50 text-center transition-all cursor-pointer relative"
                        ...
                        <p class="text-xs font-semibold text-gray-700">Click to upload sponsor logo</p>
                        <p class="text-[10px] text-gray-400 mt-1">PNG, JPG, SVG or WEBP up to 2MB</p>
                ...
                <div x-show="logoMethod === 'url'"
                     ...
                     x-transition:enter-end="opacity-100 transform translate-y-0"
                     class="space-y-2">
                    <input type="url" name="logo_url" value="{{ old('logo_url') }}" class="form-control" placeholder="https://example.com/logo.png">
                    @error('logo_url')<div class="form-error mt-2">{{ $message }}</div>@enderror
                    <div class="text-[11px] text-gray-400">Direct link to the image hosted on another server.</div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.sponsors.index') }}" class="text-gray-400 hover:text-gray-600 text-sm">Cancel</a>
        </div>
    </form>
</div>
