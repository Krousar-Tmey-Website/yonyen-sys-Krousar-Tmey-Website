...
<style>
    ...
    .image-upload-zone.dragover .iu-icon { color: #2d6fa3; }
    .image-upload-zone .iu-label { font-size: 14px; font-weight: 500; color: #475569; }
    .image-upload-zone .iu-hint { font-size: 11px; color: #94a3b8; margin-top: 4px; }
    .image-preview-card {
        display: none;
        margin-top: 12px;
        position: relative;
    ...
    .image-preview-card img { width: 100%; max-height: 200px; object-fit: cover; display: block; }
...
@section('title', 'Edit Campaign - ' . $campaign->subject)
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    {{-- Left: Form --}}
    <div>
        <form action="{{ route('admin.newsletter.campaigns.update', $campaign) }}" method="POST" enctype="multipart/form-data" id="campaignForm">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-50 flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-gray-800">Edit Campaign</h3>
                        <p class="text-[11px] text-gray-400">Update your newsletter content and settings</p>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="subject" id="subjectInput"
                                   value="{{ old('subject', $campaign->subject) }}" required maxlength="200"
                                   ...
                                   autocomplete="off">
                            <span class="char-count absolute right-3 top-1/2 -translate-y-1/2" id="charCount">{{ strlen(old('subject', $campaign->subject)) }} / 200</span>
                        </div>
                    ...
                    <div>
                        ...
                        <div id="contentEditor"
                             ...
                             data-placeholder="Edit your newsletter content...">{!! old('content', $campaign->content) !!}</div>
                        <textarea name="content" id="contentInput" class="hidden">{{ old('content', $campaign->content) }}</textarea>
                        ...
                        <p class="text-xs text-gray-400 mt-1.5 flex items-center gap-0.5">
                            ...
                            Edit your content using the toolbar. Header images appear above the content in the email.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        {{-- Current image --}}
                        @if($campaign->image)
                            <div class="mb-3 p-3 bg-gray-50 rounded-xl border border-gray-100 flex items-center gap-3" id="currentImageWrapper">
                                <img src="{{ $campaign->image_url }}" alt="Current header image"
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-gray-400 truncate">{{ basename($campaign->image) }}</p>
                                </div>
                                <label class="inline-flex items-center gap-1.5 text-xs text-red-500 hover:text-red-700 cursor-pointer select-none">
                                    <input type="checkbox" name="remove_image" value="1" class="rounded border-gray-300 text-red-500 w-3.5 h-3.5"
                        ...
                        <div class="image-upload-zone" id="imageUploadZone" style="{{ $campaign->image ? 'display:none;' : '' }}">
                            ...
                            <div class="iu-label">Drop a new image here or click to browse</div>
    ...
    <div class="lg:sticky lg:top-6 self-start">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="live-preview-wrap" id="livePreview">
                <div class="lp-header">
                    <h2 id="previewSubject">{{ $campaign->subject }}</h2>
                </div>
                <div id="previewImageArea" class="{{ $campaign->image_url ? '' : 'hidden' }}" style="line-height:0;">
                    <img id="previewImage" src="{{ $campaign->image_url ?? '' }}" alt="Header Image" style="width:100%;max-height:240px;object-fit:cover;display:block;">
                </div>
...
<script>
    ...
    function updateSubject() {
        ...
        previewSubject.textContent = val || '{{ $campaign->subject }}';
        if (charCount) {
    ...
    function handleImageFile(file) {
        ...
        reader.onload = function(e) {
            ...
            if (previewImageArea) previewImageArea.classList.remove('hidden');
        };
    ...
    removeImageBtn.addEventListener('click', function(e) {
        ...
        imageInput.value = '';
        if (previewImageArea) previewImageArea.classList.add('hidden');
    });
    ...
    function toggleRemoveImage(checkbox) {
        if (checkbox.checked) {
            ...
            document.getElementById('currentImageWrapper').style.display = 'flex';
            imageUploadZone.style.display = 'none';
        }
    }
