...
<div x-data="{ residency: '{{ request('residency', 'cambodia') }}' }" class="bg-[#f8f9fc]">
    ...
    <section class="pt-16 pb-20 bg-[#f8f9fc]">
        <div class="max-w-[1040px] mx-auto px-4 sm:px-6 2xl:px-0">
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-1.5 shadow-2xs mb-12 flex flex-wrap lg:flex-nowrap justify-between gap-1 max-w-[1040px] mx-auto">
                
                {{-- Tab: Cambodia --}}
                <button type="button"
                   @click="residency = 'cambodia'; window.history.replaceState({}, '', '?residency=cambodia')"
                   :class="residency === 'cambodia' ? 'bg-[#2d6fa3] text-white shadow-xs' : 'text-slate-655 hover:bg-slate-50 hover:text-slate-800'"
                   class="flex-1 min-w-[200px] flex items-center justify-center gap-2.5 px-4 py-3 text-xs font-black rounded-xl transition-all duration-200 select-none focus:outline-none cursor-pointer">
            ...
            <div x-show="residency !== 'cambodia'" x-cloak>
                <div class="grid md:grid-cols-2 gap-8 items-start">
                    ...
                    {{-- LEFT COLUMN: France Ways to Donate OR Standard Impact Description --}}
                    <div>
                        {{-- France left-column content: Ways to Donate --}}
                        <div x-show="residency === 'france'" class="space-y-6">
                            <div class="bg-white rounded-2xl border border-slate-200/80 p-8 shadow-sm space-y-6">
                                <div class="space-y-3">
                                    <span class="text-xs font-bold text-[#2d6fa3] tracking-widest uppercase block">Support Us</span>
                                    <h2 class="text-3xl font-black text-slate-800 leading-tight">Ways to Donate</h2>
                                    <p class="text-xs text-slate-500 leading-relaxed">
                                        You can support Krousar Thmey France's work through one of the following methods:
                                    </p>
                                </div>

                                {{-- HelloAsso --}}
                                <div class="bg-slate-50 rounded-xl p-5 border border-slate-200/60 space-y-3">
                                    ...
                                    <a href="https://www.helloasso.com/associations/krousar-thmey" target="_blank" rel="noopener"
                                       class="inline-flex w-full items-center justify-center gap-2 px-5 py-3 bg-[#2d6fa3] hover:bg-[#1d4e7a] text-white text-xs font-bold rounded-xl shadow-xs transition-all text-center">
                                        <span>HelloAsso (One-time or regular donation)</span>
                                ...
                                <div class="bg-slate-50 rounded-xl p-5 border border-slate-200/60 space-y-3">
                                    ...
                                    <div class="bg-white border border-slate-200 rounded-xl p-4 text-xs font-mono text-slate-755 space-y-1 shadow-2xs">
                                        <div class="font-bold text-[#2d6fa3]">Krousar Thmey France</div>
                                        <div>62 rue Greneta</div>
                                        <div>75002 Paris</div>
                                    </div>
                                </div>
                            </div>
                        ...
                        {{-- Non-France left-column content: standard impact description & bullet points --}}
                        <div x-show="residency !== 'france'" class="bg-white rounded-2xl border border-slate-200/80 p-8 shadow-sm flex flex-col justify-between min-h-[460px]">
                            <div class="space-y-6">
                                <div class="text-center md:text-left space-y-3">
                                    ...
                                    <div x-show="residency === 'switzerland'" class="space-y-3">
                                        <p class="text-sm text-slate-550 leading-relaxed">
                                            Donating through <strong class="text-slate-800">Krousar Thmey Switzerland</strong> is tax-deductible according to Swiss canton regulations. We will issue a tax donation certificate to claim deductions on your Swiss tax declaration.
                                        </p>
                                    </div>
                                    
                                    {{-- Elsewhere description --}}
                                    <div x-show="residency === 'elsewhere'" class="space-y-3">
                                        <p class="text-sm text-slate-550 leading-relaxed">
                    ...
                    {{-- RIGHT COLUMN: France Tax/Legacy Accordion OR Standard Dual-step Wizard --}}
                    <div>
                        {{-- France right-column content: Tax Exemptions & Legacy info --}}
                        <div x-show="residency === 'france'" 
                             x-cloak
                             x-data="{ activeSection: 'tax' }" 
                             class="space-y-6">
                            
                            {{-- Tax deductions card --}}
                            <div class="bg-white rounded-2xl border border-slate-200/80 p-8 shadow-sm space-y-6">
                                <button type="button" @click="activeSection = (activeSection === 'tax' ? '' : 'tax')"
                                        class="w-full flex items-center justify-between focus:outline-none text-left cursor-pointer">
                                    <div class="space-y-2">
                                        <span class="text-xs font-bold text-[#2d6fa3] tracking-widest uppercase block">Tax Exemption</span>
                                        <h2 class="text-2xl font-black text-slate-800 tracking-wide uppercase">Tax Deductions</h2>
                                    </div>
                                    <svg class="w-5 h-5 text-slate-500 transition-transform duration-200" :class="activeSection === 'tax' ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                                
                                <div x-show="activeSection === 'tax'" x-collapse class="space-y-4 pt-4 border-t border-slate-100">
                                    <p class="text-xs text-slate-500 leading-relaxed font-semibold">
                            ...
                            <div class="bg-white rounded-2xl border border-slate-200/80 p-8 shadow-sm space-y-6">
                                <button type="button" @click="activeSection = (activeSection === 'legacy' ? '' : 'legacy')"
                                        class="w-full flex items-center justify-between focus:outline-none text-left cursor-pointer">
                                    <div class="space-y-2">
                                        <span class="text-xs font-bold text-[#2d6fa3] tracking-widest uppercase block">Estate Planning</span>
                                        <h2 class="text-2xl font-black text-slate-800 tracking-wide uppercase">Legacy</h2>
                                    </div>
                                    <svg class="w-5 h-5 text-slate-500 transition-transform duration-200" :class="activeSection === 'legacy' ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                                
                                <div x-show="activeSection === 'legacy'" x-collapse class="space-y-4 pt-4 border-t border-slate-100" x-data="{ legacyType: 'bequest' }">
                                    <p class="text-xs text-slate-500 leading-relaxed font-semibold">
                                    ...
                                    <div class="flex gap-2 border-b border-slate-100 pb-3">
                                        <button type="button" @click="legacyType = 'bequest'"
                                                :class="legacyType === 'bequest' ? 'bg-[#8da83a] text-white shadow-xs' : 'bg-slate-100 text-slate-650 hover:bg-slate-200/60'"
                                        ...
                                        <button type="button" @click="legacyType = 'donation'"
                                                :class="legacyType === 'donation' ? 'bg-[#8da83a] text-white shadow-xs' : 'bg-slate-100 text-slate-650 hover:bg-slate-200/60'"
                                                class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all focus:outline-none cursor-pointer">
                                            Donation (Lifetime)
                                        </button>
                                    </div>
                        ...
                        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden min-h-[460px]"
                            ...
                            <div x-show="step === 1" class="flex flex-col h-full justify-between">
                                ...
                                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                                    ...
                                    <span class="bg-slate-100 text-slate-650 px-2 py-0.5 rounded text-[10px] font-bold" x-text="currency"></span>
                                </div>
                ...
                <div x-show="residency === 'france'" x-cloak class="mt-12">
                    <p class="text-[11px] text-slate-400 leading-relaxed italic text-center max-w-2xl mx-auto">
