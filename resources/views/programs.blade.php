    ...
    <div class="max-w-7xl mx-auto px-6">
        <div class="grid lg:grid-cols-12 gap-10 items-stretch">
            ...
            <div class="lg:col-span-6 xl:col-span-7 flex flex-col justify-center pr-4">
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <h2 class="text-3xl lg:text-4xl font-black text-[#1a3c6e] uppercase tracking-wide leading-tight">{{ $program->title }}</h2>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <p class="text-gray-700 leading-relaxed text-[15px] whitespace-pre-line">{{ $program->full_description }}</p>
                </div>
                @endif

                <div class="flex flex-col sm:flex-row gap-4 mb-8">
                    @if($program->projects && $program->projects->count() > 0)
                    <a href="#projects-{{ $program->slug }}" class="bg-[#1a3c6e] text-white px-6 py-3 rounded text-sm font-semibold hover:bg-[#1d4e7a] transition-colors text-center shadow-sm">
                        Know more about the projects
                    </a>
                    @endif
                    <a href="{{ route('donate') }}" class="bg-white text-[#d32f2f] border border-[#d32f2f] px-6 py-3 rounded text-sm font-semibold hover:bg-[#d32f2f] hover:text-white transition-colors text-center flex items-center justify-center gap-2 shadow-sm uppercase tracking-wider">
                        DONATE NOW
                    </a>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ $program->facebook_url }}" target="_blank" rel="noopener" class="w-9 h-9 text-white rounded bg-[#1877f2] flex items-center justify-center hover:opacity-90 transition-opacity">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    </a>
                    @endif
                    @if($program->telegram_url)
                    <a href="{{ $program->telegram_url }}" target="_blank" rel="noopener" class="w-9 h-9 text-white rounded bg-[#1da1f2] flex items-center justify-center hover:opacity-90 transition-opacity">
                    ...
                    <a href="{{ $program->linkedin_url }}" target="_blank" rel="noopener" class="w-9 h-9 text-white rounded bg-[#0a66c2] flex items-center justify-center hover:opacity-90 transition-opacity">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                    </a>
                    @endif
                    <div class="w-9 h-9 text-white rounded bg-[#0066ff] flex items-center justify-center hover:opacity-90 transition-opacity cursor-pointer">
        ...
        <div class="mt-16 bg-[#e9ebed] p-10 md:p-14 text-center max-w-5xl mx-auto shadow-sm">
            <div class="relative inline-block mb-4">
                ...
                <img src="{{ str_starts_with($program->testimony_image, 'http') ? $program->testimony_image : asset('storage/' . $program->testimony_image) }}" class="w-32 h-32 mx-auto rounded-full object-cover relative z-10" alt="Testimony">
                @else
                <div class="w-32 h-32 mx-auto rounded-full bg-white flex items-center justify-center text-[#1a3c6e] font-bold text-4xl relative z-10">{{ substr($cleanTestimonyName, 0, 1) }}</div>
                @endif
                {{-- Quote Icon Badge --}}
                <div class="absolute -top-1 right-1 z-20 w-8 h-8 bg-white rounded-full flex items-center justify-center">
            ...
            <div x-data="{ open: true }" class="bg-white text-left mx-auto max-w-3xl">
                <button @click="open = !open" class="w-full flex items-center justify-between px-6 py-4 hover:bg-gray-50 focus:outline-none cursor-pointer border border-gray-200">
                    <span class="text-gray-400 font-semibold text-sm">Read her story</span>
                    <svg class="w-5 h-5 text-gray-400 transform transition-transform duration-300" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div x-show="open" class="px-6 pb-6 pt-4 border-l border-r border-b border-gray-200" style="display: none;"
                     x-transition:enter="transition ease-out duration-200"
                     x-transition:enter-start="opacity-0"
                     x-transition:enter-end="opacity-100">
                    <p class="text-gray-700 leading-relaxed text-[15px] whitespace-pre-line italic">{{ $program->testimony_story }}</p>
                </div>
            </div>
        </div>
        @endif
        ...
        <div id="projects-{{ $program->slug }}" class="mt-20">
            <h3 class="text-sm font-bold text-[#1a3c6e] uppercase tracking-wide mb-8 text-center sm:text-left">DISCOVER THE PROJECTS OF THIS PROGRAM</h3>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-white border border-gray-200 flex flex-col h-full hover:shadow-lg transition-shadow">
                    ...
                    <div class="p-6 flex flex-col flex-1 items-center text-center">
                        <h4 class="text-[13px] font-bold text-[#1a3c6e] uppercase tracking-wide mb-3 leading-relaxed">{{ $project->title }}</h4>
                        @if($project->description)
                        <p class="text-gray-500 text-xs leading-relaxed flex-1 mb-6">{{ Str::limit($project->description, 120) }}</p>
                        @endif
                        ...
                        <div class="w-full mt-auto flex flex-col gap-3 max-w-[200px]">
                            <a href="{{ route('donate') }}"
                               class="w-full inline-flex justify-center items-center gap-1 text-xs font-bold text-[#d32f2f] bg-white border border-[#d32f2f] hover:bg-[#d32f2f] hover:text-white transition-colors py-2.5">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M7 17L17 7M17 7H7M17 7V17"/></svg>
                                DONATE NOW
                            </a>
                            <a href="{{ route('projects.show', $project) }}" class="w-full inline-block bg-[#1a3c6e] text-white text-xs font-bold text-center py-2.5 hover:bg-[#1d4e7a] transition-colors">
                                Read more
                            </a>
                        </div>
                    </div>
