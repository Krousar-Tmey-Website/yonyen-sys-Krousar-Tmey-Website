    ...
    <section class="bg-[#1a3c6e] relative overflow-hidden min-h-[560px] md:min-h-[620px] lg:min-h-[680px] flex items-center">
        ...
        <div class="relative w-full max-w-7xl mx-auto px-6 py-16 md:py-20 lg:py-24 z-10">
            <div class="max-w-[38rem]">
                ...
                <h1 class="text-5xl md:text-6xl lg:text-7xl font-black text-white mb-6 uppercase tracking-wide leading-tight drop-shadow-lg hero-reveal hero-reveal-delay-1">
                    {{ $bannerTitle }}
                </h1>
                ...
                @foreach($programs->take(3) as $index => $prog)
                    <a href="#{{ $prog->slug }}"
                        ...
                        <div class="relative flex items-center justify-between">
                            ...
                            @if($prog->Status)
                                <span class="text-[10px] font-black uppercase tracking-widest text-white/90 bg-white/20 backdrop-blur px-3 py-1.5 rounded-full">{{ $prog->Status }}</span>
                            @endif
                        </div>
                        <div class="relative mt-8">
                            <div class="font-black text-white text-lg uppercase tracking-wider drop-shadow-sm">{{ $prog->title }}</div>
        ...
        <section id="{{ $program->slug }}" class="py-20 lg:py-28 {{ $isEven ? 'bg-gray-50' : 'bg-white' }} relative overflow-hidden">
            ...
            <div class="max-w-7xl mx-auto px-6 relative z-10">
                <div class="grid lg:grid-cols-12 gap-12 lg:gap-20 items-center">
                    ...
                    <div class="lg:col-span-6 {{ $isEven ? 'lg:order-2' : '' }} flex flex-col justify-center">
                        <div class="flex items-center gap-4 mb-8">
                            <div class="w-2 h-14 rounded-full bg-gradient-to-b from-[#d32f2f] to-[#e8a020]"></div>
                            <h2 class="text-4xl lg:text-5xl font-black text-[#1a3c6e] uppercase tracking-wide leading-tight drop-shadow-sm">
                                {{ $program->title }}
                            </h2>
                        </div>
                        ...
                        @if($program->full_description)
                            <div class="mb-10 bg-white/50 backdrop-blur rounded-3xl p-6 border border-gray-100 shadow-sm">
                                ...
                                <div class="prose prose-lg prose-p:text-gray-600 prose-p:leading-loose max-w-none">
                                    <p class="whitespace-pre-line">{{ $program->full_description }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 mb-10">
                            ...
                            @endif
                            <a href="{{ route('donate') }}"
                                class="bg-white text-[#d32f2f] border-2 border-[#d32f2f] px-8 py-4 rounded-xl text-sm font-bold hover:bg-[#d32f2f] hover:text-white hover:shadow-lg transition-all text-center flex items-center justify-center gap-2 uppercase tracking-widest">
                                Donate Now
                            </a>
                        </div>
                    </div>
                    ...
                    {{-- Right Side: Image --}}
                    <div class="lg:col-span-6 {{ $isEven ? 'lg:order-1' : '' }} relative hidden md:block group">
                        <div class="absolute inset-0 bg-[#1a3c6e]/5 rounded-[3rem] transform rotate-3 scale-105 group-hover:rotate-6 transition-transform duration-700"></div>
                        <div class="relative w-full h-[500px] rounded-[3rem] overflow-hidden shadow-2xl shadow-[#1a3c6e]/20 border-[8px] border-white z-10">
                ...
                @if($program->testimony_name && $program->testimony_story)
                    @php $cleanTestimonyName = preg_replace('/^testimony\s*:?\s*/i', '', $program->testimony_name); @endphp
                    <div class="mt-24 max-w-5xl mx-auto bg-gradient-to-br from-[#1a3c6e] to-[#2d6fa3] rounded-[3rem] p-10 md:p-16 text-center shadow-2xl relative overflow-hidden group">
                    ...
                    <div id="projects-{{ $program->slug }}" class="mt-28">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            @foreach($program->projects as $project)
                                <a href="{{ route('projects.show', $project) }}" class="group bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 border border-gray-100 flex flex-col h-full relative">
                                    ...
                                    <div class="h-56 overflow-hidden relative">
                                        ...
                                        <div class="absolute bottom-6 left-6 right-6">
                                            <h4 class="text-xl font-black text-white uppercase tracking-wide leading-snug drop-shadow-md">{{ $project->title }}</h4>
                                        </div>
                                    </div>
            ...
            <div class="max-w-7xl mx-auto px-6 relative z-10">
                <div class="text-center mb-16" data-reveal>
                    <div class="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-[#e8a020]/10 border border-[#e8a020]/20 mb-5 shadow-sm">
                        ...
                        <span class="text-[#e8a020] font-bold text-xs uppercase tracking-widest">Cross-cutting Work</span>
                    </div>
                    <h2 class="text-4xl md:text-5xl font-black text-[#1a3c6e] uppercase tracking-wide">Additional Programs</h2>
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    @foreach($programs->skip(3) as $index => $program)
                        <div class="group bg-white rounded-[2rem] p-10 border border-gray-100 hover:border-transparent hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 relative overflow-hidden" data-reveal="up" style="--reveal-delay: {{ $index * 100 }}">
                            ...
                            <div class="w-16 h-16 rounded-2xl {{ $extraColors[$index % 3] }} flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-300 relative z-10">
                                {!! $programIconFor($program->title, 'w-8 h-8') !!}
                            </div>
                            ...
                            @if($program->Status)
                                <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#e8a020]/10 border border-[#e8a020]/20 mb-4 relative z-10">
                                    <span class="text-[#e8a020] font-black text-[10px] uppercase tracking-widest">{{ $program->Status }}</span>
                                </div>
                            @endif
                            
                            <h3 class="text-2xl font-black text-[#1a3c6e] uppercase tracking-wide mb-4 relative z-10">{{ $program->title }}</h3>
                            <p class="text-gray-600 text-[15px] leading-relaxed whitespace-pre-line relative z-10 font-medium">{{ $program->description }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="max-w-7xl mx-auto px-6 relative z-10">
                <div class="text-center mb-16" data-reveal>
                    <div class="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-[#2d6fa3]/10 border border-[#2d6fa3]/20 mb-5 shadow-sm">
                        ...
                        <span class="text-[#2d6fa3] font-bold text-xs uppercase tracking-widest">Learn More</span>
                    </div>
                    <h2 class="text-4xl md:text-5xl font-black text-[#1a3c6e] uppercase tracking-wide">Additional Information</h2>
                    <div class="w-24 h-1.5 bg-gradient-to-r from-[#d32f2f] to-[#e8a020] mx-auto mt-6 rounded-full"></div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($additionalItems as $item)
                        <a href="{{ route('program-page-items.show', $item->id) }}"
                            ...
                            @if($item->image)
                                <div class="h-60 overflow-hidden relative bg-[#1a3c6e]/5 p-2">
                                    <img src="{{ $item->image_url }}" alt="{{ $item->title }}" class="w-full h-full object-cover rounded-3xl group-hover:scale-105 transition-transform duration-700 shadow-inner">
                                </div>
                            @else
                                <div class="h-48 bg-gradient-to-br from-[#1a3c6e] to-[#2d6fa3] flex items-center justify-center m-2 rounded-3xl">
                            ...
                            <div class="p-8 flex flex-col flex-1 relative z-10">
                                <h3 class="text-xl font-black text-[#1a3c6e] mb-4 uppercase tracking-wide group-hover:text-[#2d6fa3] transition-colors">{{ $item->title }}</h3>
                                
                                @if($item->short_content)
                                    <p class="text-gray-600 text-[15px] font-medium leading-relaxed flex-1 mb-8">{{ Str::limit($item->short_content, 130) }}</p>
                                @endif
    ...
    <section class="relative py-24 overflow-hidden bg-[#1a3c6e]">
        ...
        <div class="relative max-w-4xl mx-auto px-6 text-center z-10" data-reveal="scale">
            <div class="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-white/10 backdrop-blur border border-white/20 mb-8 shadow-sm">
                ...
                <span class="text-white font-bold text-xs uppercase tracking-widest">Support Our Mission</span>
            </div>
            
            <h2 class="text-4xl md:text-6xl font-black text-white uppercase tracking-wide mb-6 drop-shadow-md">Help Children in Cambodia</h2>
            
            <p class="text-white/80 text-lg md:text-xl mb-12 max-w-2xl mx-auto leading-relaxed font-medium">Your donation goes directly to one of these programs. 100% of funds support children in Cambodia.</p>
            
            <div class="flex flex-col sm:flex-row gap-5 justify-center items-center">
                <a href="{{ route('donate') }}" class="w-full sm:w-auto px-10 py-4 bg-[#8da83a] hover:bg-[#a3c04a] text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all uppercase tracking-widest flex items-center justify-center gap-3">
