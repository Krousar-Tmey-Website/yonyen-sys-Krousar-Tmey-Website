...
{{-- Main Content with shared Alpine.js state --}}
<div x-data="{ active: '{{ request('category', 'all') }}' }">
    {{-- Filter Tabs --}}
    <section class="bg-white border-b border-gray-100 sticky top-[64px] lg:top-[80px] z-40">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex items-center gap-1 overflow-x-auto py-3 scrollbar-hide">
                <button @click="active = 'all'; window.history.pushState({}, '', '{{ route('news') }}')"
                ...
                <button @click="active = '{{ $category->CategoryName }}'; window.history.pushState({}, '', '{{ route('news', ['category' => $category->CategoryName]) }}')"
                        :class="active === '{{ $category->CategoryName }}' ? 'bg-[#1a3c6e] text-white' : 'text-gray-600 hover:bg-gray-50'"
                        class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 flex-shrink-0">
                ...
                <a href="{{ route('news.show', $article->slug) }}" class="block">
                    <article class="card group flex flex-col"
                        ...
                        <div class="relative overflow-hidden h-52">
                            ...
                            <span class="absolute top-4 left-4 bg-white/90 backdrop-blur-sm text-[#1a3c6e] text-xs font-semibold px-3 py-1 rounded-full">{{ $article->category_name }}</span>
                        </div>
