    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        regions.forEach(region => {
            region.addEventListener("mouseenter", (e) => {
                ...
                tooltip.textContent = formatProvinceName(province);
        ...
        if (markersContainer && markerTooltip) {
            const svgNs = 'http://www.w3.org/2000/svg';
            const markers = [];

            locationData.forEach((loc, index) => {
                const g = document.createElementNS(svgNs, 'g');
                g.setAttribute('class', 'project-marker');
                g.style.transitionDelay = (index * 0.08) + 's';

                const pulse = document.createElementNS(svgNs, 'circle');
                pulse.setAttribute('cx', loc.cx); pulse.setAttribute('cy', loc.cy);
                pulse.setAttribute('r', 8); pulse.setAttribute('fill', 'none');
                pulse.setAttribute('stroke', '#0F766E'); pulse.setAttribute('stroke-width', '2');
                pulse.classList.add('marker-pulse');
                g.appendChild(pulse);

                const outer = document.createElementNS(svgNs, 'circle');
                outer.setAttribute('cx', loc.cx); outer.setAttribute('cy', loc.cy);
                outer.setAttribute('r', 5.5); outer.setAttribute('fill', '#0F766E');
                outer.setAttribute('stroke', 'white'); outer.setAttribute('stroke-width', '2');
                g.appendChild(outer);

                const inner = document.createElementNS(svgNs, 'circle');
                inner.setAttribute('cx', loc.cx); inner.setAttribute('cy', loc.cy);
                inner.setAttribute('r', 4); inner.setAttribute('fill', 'white');
                g.appendChild(inner);

                const dot = document.createElementNS(svgNs, 'circle');
                dot.setAttribute('cx', loc.cx); dot.setAttribute('cy', loc.cy);
                dot.setAttribute('r', 2.5); dot.setAttribute('fill', '#F97316');
                g.appendChild(dot);
                ...
                g.addEventListener('mouseenter', () => {
                    ...
                    markerTooltip.innerHTML = `
                        <div class="tt-header">
                            <span class="tt-title">${loc.name}</span>
                            <span class="tt-badge">${loc.projects.length} program${loc.projects.length > 1 ? 's' : ''}</span>
                        </div>
                        <div class="tt-body">
                            <div class="tt-sub">
                                ${pinSvg}
                            ...
                            <div class="tt-tags">
                                ${loc.projects.map(p => `<span class="tt-tag">${p}</span>`).join('')}
                            </div>
                        </div>
                    `;
                ...
                g.addEventListener('mousemove', (e) => {
                    ...
                    const tooltipW = markerTooltip.offsetWidth;
                    const tooltipH = markerTooltip.offsetHeight;

                    // Horizontal bounds
                    if (x + tooltipW / 2 > window.innerWidth - 12) {
                        x = window.innerWidth - tooltipW / 2 - 12;
                    } else if (x - tooltipW / 2 < 12) {
                        x = tooltipW / 2 + 12;
                    }

                    // Vertical bounds - flip below if too close to top
                    if (y - tooltipH - 16 < 8) {
                        y = y + 14; // show below cursor
                        markerTooltip.style.transform = 'translate(-50%, 0%)';
                        markerTooltip.classList.add('flip-below');
                    } else {
                        y = y - 10;
                        markerTooltip.style.transform = 'translate(-50%, -100%)';
                        markerTooltip.classList.remove('flip-below');
                    }
                ...
                markersContainer.appendChild(g);
                markers.push(g);
            });
            ...
            // Observe the map container for entrance
            const mapContainer = markersContainer.closest('.map-svg-pn') || markersContainer;
            markerObserver.observe(mapContainer);
        }
...
<style>
    ...
    .project-marker.entered {
        opacity: 1;
        transform: scale(1);
    }

    .project-marker circle {
        transition: transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1);
        transform-box: fill-box;
        transform-origin: 50% 50%;
    }

    .project-marker:hover circle {
        transform: scale(1.35);
    }

    .project-marker:hover .marker-pulse {
    ...
    /* Flip arrow upward when tooltip appears below cursor */
    .marker-tooltip.flip-below::after {
        bottom: auto;
        top: -7px;
        border-top: none;
    ...
    .marker-tooltip.flip-below::before {
        bottom: auto;
        top: -5px;
        border-top: none;
    ...
    .marker-tooltip .tt-body {
        padding: 10px 14px 12px;
    }

    .marker-tooltip .tt-sub {
        color: #6b7280;
        font-size: 11px;
        font-weight: 500;
        margin-bottom: 6px;
    ...
    .marker-tooltip .tt-sub svg {
        width: 12px;
        height: 12px;
        flex-shrink: 0;
    }

    .marker-tooltip .tt-tags {
        display: flex;
        gap: 5px;
        flex-wrap: wrap;
    }

    .marker-tooltip .tt-tag {
    ...
    /* ── Map Province Hover Effects ───────────────── */

    /* Default: green for active provinces */
    .map-svg-cambodia .region {
        fill: #8da83a !important;
        cursor: pointer;
        transition: fill 0.25s ease;
    }

    .map-svg-cambodia .region:hover {
        fill: #6b8a23 !important;
    }

    /* No-project provinces (gray) - no hover color change */
    .map-svg-cambodia .region[data-region="uddor-meanchey"],
    .map-svg-cambodia .region[data-region="preah-vihear"],
    .map-svg-cambodia .region[data-region="stung-treng"],
    .map-svg-cambodia .region[data-region="ratanak-kiri"],
    .map-svg-cambodia .region[data-region="kratie"],
    .map-svg-cambodia .region[data-region="mondulkiri"],
    .map-svg-cambodia .region[data-region="pailin"],
    .map-svg-cambodia .region[data-region="koh-kong"],
    .map-svg-cambodia .region[data-region="preah-sihanouk"],
    .map-svg-cambodia .region[data-region="kampot"],
    .map-svg-cambodia .region[data-region="kampong-chhnang"] {
        fill: #d4d4d4 !important;
        cursor: default;
    }

    .map-svg-cambodia .region[data-region="uddor-meanchey"]:hover,
    .map-svg-cambodia .region[data-region="preah-vihear"]:hover,
    .map-svg-cambodia .region[data-region="stung-treng"]:hover,
    .map-svg-cambodia .region[data-region="ratanak-kiri"]:hover,
    .map-svg-cambodia .region[data-region="kratie"]:hover,
    .map-svg-cambodia .region[data-region="mondulkiri"]:hover,
    .map-svg-cambodia .region[data-region="pailin"]:hover,
    .map-svg-cambodia .region[data-region="koh-kong"]:hover,
    .map-svg-cambodia .region[data-region="preah-sihanouk"]:hover,
    .map-svg-cambodia .region[data-region="kampot"]:hover,
    .map-svg-cambodia .region[data-region="kampong-chhnang"]:hover {
        fill: #d4d4d4 !important;
    }
...
<div class="map-tooltip" id="mapTooltip">Province</div>
