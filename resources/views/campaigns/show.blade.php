...
<section class="relative overflow-hidden bg-[#1d4e7a]">
    ...
    <img src="{{ $campaign->image_url }}" alt="" aria-hidden="true"
         class="absolute inset-0 w-full h-full object-cover object-center scale-105 blur-sm opacity-40">
    @endif
    <div class="absolute inset-0 bg-gradient-to-b from-[#12324f]/90 to-[#1d4e7a]/95"></div>

    <div class="relative max-w-5xl mx-auto px-6 py-14 lg:py-16">
        <nav data-reveal class="flex items-center flex-wrap gap-2 text-sm text-white/50 mb-7">
            <a href="{{ route('home') }}" class="hover:text-white transition-colors">{{ __('Home') }}</a>
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <a href="{{ route('campaigns.index') }}" class="hover:text-white transition-colors">{{ __('Campaigns') }}</a>
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        ...
        <div data-reveal style="--reveal-delay: 60" class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#8da83a]/25 border border-[#8da83a]/40 mb-5">
            <svg class="w-3.5 h-3.5 text-[#cfe08a]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            <span class="text-[#cfe08a] font-bold text-xs uppercase tracking-widest">{{ $campaign->year }}</span>
        </div>
        @endif

        <h1 data-reveal style="--reveal-delay: 120" class="text-3xl md:text-4xl lg:text-5xl font-black text-white leading-tight max-w-3xl">
            {{ $campaign->localized_title }}
        </h1>

        <div data-reveal style="--reveal-delay: 180" class="flex flex-wrap items-center gap-3 mt-8">
            <a href="{{ route('donate') }}"
               class="inline-flex items-center gap-2 bg-[#8da83a] hover:bg-[#a3c04a] text-white px-7 py-3.5 rounded-full font-semibold text-sm shadow-lg shadow-black/20 hover:-translate-y-0.5 transition-all">
...
{{-- ── Body ───────────────────────────────────────────────── --}}
<section class="bg-[#f8f9fc] py-12 lg:py-16">
    <div class="max-w-5xl mx-auto px-6 space-y-8">
        ...
        <div data-reveal class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="bg-black aspect-video">
                @if($embedUrl)
                <iframe src="{{ $embedUrl }}" title="{{ $campaign->localized_title }}"
                ...
                <a href="{{ $campaign->video_url }}" target="_blank" rel="noopener"
                   class="w-full h-full flex flex-col items-center justify-center gap-3 text-white/80 hover:text-white transition-colors">
                    <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664zM21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <span class="text-sm font-semibold">{{ __('Watch the video') }}</span>
                </a>
                @else
                <video controls preload="metadata" class="w-full h-full"
                       @if($campaign->has_image) poster="{{ $campaign->image_url }}" @endif>
        ...
        <article data-reveal class="bg-white rounded-3xl border border-gray-100 shadow-sm p-7 md:p-10">
            <div class="flex items-center gap-3 pb-5 mb-6 border-b border-gray-100">
                <span class="w-9 h-9 rounded-xl bg-[#8da83a]/12 flex items-center justify-center flex-shrink-0">
                    <svg class="w-4.5 h-4.5 text-[#8da83a]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h10"/></svg>
                </span>
                <div>
        ...
        <div data-reveal class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <object data="{{ $campaign->file_url }}" type="application/pdf" class="w-full h-[38rem] bg-gray-50">
                <div class="flex flex-col items-center justify-center h-full gap-3 text-gray-400 p-8 text-center">
                    ...
                    <a href="{{ $campaign->file_url }}" target="_blank" rel="noopener" class="text-[#2d6fa3] font-semibold text-sm hover:underline">{{ __('Open it in a new tab') }}</a>
                </div>
            </object>
            @endif
        </div>
        ...
        {{-- Inline donate CTA --}}
        <div data-reveal class="relative overflow-hidden rounded-3xl bg-[#1d4e7a] px-7 py-9 md:px-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="absolute top-0 right-0 w-64 h-64 rounded-full bg-[#8da83a]/20 -translate-y-1/2 translate-x-1/3"></div>
            <div class="relative">
                <h2 class="text-xl md:text-2xl font-black text-white mb-1.5">{{ __('Support this campaign') }}</h2>
                <p class="text-white/60 text-sm leading-relaxed">{{ __('100% of your donation directly supports children in Cambodia.') }}</p>
            </div>
            <a href="{{ route('donate') }}"
...
<section class="bg-white py-16 border-t border-gray-100">
    <div class="max-w-7xl mx-auto px-6">
        <div data-reveal class="flex items-end justify-between gap-6 mb-10">
            <div>
                <span class="text-[#8da83a] font-bold text-xs uppercase tracking-widest">{{ __('Keep Exploring') }}</span>
                <h2 class="text-2xl md:text-3xl font-black text-[#1a3c6e] mt-2">{{ __('More Campaigns') }}</h2>
            </div>
            <a href="{{ route('campaigns.index') }}" class="hidden sm:inline-flex items-center gap-1.5 text-[#2d6fa3] text-sm font-bold hover:gap-2.5 transition-all flex-shrink-0 pb-1">
                {{ __('View all') }}
